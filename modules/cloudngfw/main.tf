...
resource "azurerm_palo_alto_next_generation_firewall_virtual_network_strata_cloud_manager" "this" {
  count = var.attachment_type == "vnet" && var.management_mode == "scm" ? 1 : 0
  ...
  plan_id                          = var.cloudngfw_config.plan_id
  marketplace_offer_id             = var.cloudngfw_config.marketplace_offer_id
  strata_cloud_manager_tenant_name = var.cloudngfw_config.strata_cloud_manager_tenant_name
...
resource "azurerm_palo_alto_next_generation_firewall_virtual_hub_strata_cloud_manager" "this" {
  count = var.attachment_type == "vwan" && var.management_mode == "scm" ? 1 : 0
